...
public class CtlGenerator {
	...
	CtlGenerator(int onum, int pownum, int propnum) throws IOException, InterruptedException {
		...
		atoms = "randltl -n" + prop_number
				+ " -p --seed=4 --ltl-priorities 'false=0,true=0,not=2,and=4,or=1,xor=0,implies=2,equiv=0,W=0,M=0,R=0,X=0,U=0,F=3,G=2' --tree-size=7 -o "
				+ tmp_file + " ";
	...
	private String ReplaceU(String content) {
		...
		while (occurance_num > 0) {
			...
			Random rand = new Random();
			if (rand.nextInt(2) == 0)
				content = content.substring(0, l_cursor + 1) + "A[" + content.substring(l_cursor + 1, r_cursor) + "]"
						+ content.substring(r_cursor);
			else
				content = content.substring(0, l_cursor + 1) + "E[" + content.substring(l_cursor + 1, r_cursor) + "]"
						+ content.substring(r_cursor);
			occurance_num--;
		}
		return content;
